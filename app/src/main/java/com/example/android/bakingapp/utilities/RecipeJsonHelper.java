...
public class RecipeJsonHelper {
    ...
    public static String getIngredientsStringForRecipe(Recipe recipeObj) {
        ...
        objString.append(INGREDIENTS).append(" : ").append(recipeObj.getName()).append(" .\n");
    ...
    public static Recipe getRecipeObject(int id, Context context) {
        loadData(context);
    ...
    public static Step getRecipeStepJsonObject(int stepPosition, Recipe recipeObj) {
        return recipeObj.getSteps().get(stepPosition-1);
    }

    //Get a string[] of description on steps for recipe at position, 'id'
    public static String[] getRecipeStepDescriptions(int id, Context context) {
        loadData(context);
    ...
    public static int getNumberOfRecipeSteps(int id, Activity activity) {
        loadData(activity);
        return recipes.get(id).getSteps().size();
    }

    //Get Titles of all recipes as a String[]
    //Doesn't call loadData()
...
//        loadData();
    ...
    //Get Titles of all recipes as a String[]
    //Doesn't call loadData()
    public static String[] getImageUris() {
//        loadData();
        int len = recipes.size();
        String[] dataSet = new String[len];
        for (int i = 0; i < len; i++) {
            dataSet[i] = recipes.get(i).getImage();
        }
        return dataSet;
    }
    ...
    public static void loadData(Context context) {
        if (recipes == null) {
            ...
            String jsonStr = preferences.getString(JSON_STRING, "");
//            Log.i("RecipeHelper", jsonStr + "blah");
            if(!jsonStr.equals("")){
                Gson gson = new Gson();
                recipes = gson.fromJson(jsonStr, new TypeToken<List<Recipe>>(){}.getType());
                Log.i("RecipeJsonHelper", "load data from prefs");
            }else{
                loadRecipesObjAfterNetworkCall(context);
                Log.i("RecipeJsonHelper", "load data from network call");
            }
        }else
        Log.i("RecipeJsonHelper", "recipe is not null");
    }

    //Async call later...

    public static void loadRecipesObjAfterNetworkCall(final Context context) {
        if(recipes == null){
    ...
    public static void loadAdapterDataAfterNetworkCall(final HomePageAdapter adapter, final Activity activity) {
        if(recipes == null){
            Log.i("RecipeJsonHelper", "API call");
            RecipeService mService = ApiUtils.getRecipeService();
            mService.getRecipes().enqueue(new Callback<List<Recipe>>() {
                ...
                public void onResponse(Call<List<Recipe>> call, Response<List<Recipe>> response) {
                    if (response.isSuccessful()) {
                        ...
                        adapter.updateData(getRecipeTitles(), getImageUris());
                        Log.d("MainActivity", "posts loaded from API");
                    } else {
                        int statusCode = response.code();
                        // handle request errors depending on status code
    ...
    public static void loadFromJsonStringForTest(){
        Gson gson = new Gson();
        recipes = gson.fromJson(recipeJsonString, new TypeToken<List<Recipe>>(){}.getType());
    }
